...
def test_ensure_user_registry_loaded_loads_saved_dtd(dtd_user: UserContext):
    ...
    shared.joinpath("main.dtd").write_bytes((FIXTURES / "main.dtd").read_bytes())
    shared.joinpath("types.dtd").write_bytes((FIXTURES / "types.dtd").read_bytes())
...
def test_ensure_user_registry_loaded_loads_all_entry_points(dtd_user: UserContext):
    ...
    shared.joinpath("main.dtd").write_bytes((FIXTURES / "main.dtd").read_bytes())
    shared.joinpath("types.dtd").write_bytes((FIXTURES / "types.dtd").read_bytes())
    shared.joinpath("old-schema.dtd").write_text(
...
def test_ensure_user_registry_removes_unreferenced_files(dtd_user: UserContext):
    ...
    shared.joinpath("main.dtd").write_bytes((FIXTURES / "main.dtd").read_bytes())
    shared.joinpath("types.dtd").write_bytes((FIXTURES / "types.dtd").read_bytes())
    shared.joinpath("stale-module.ent").write_text(
        "<!ENTITY % Boolean \"(true|false)\">", encoding="utf-8"
    )

    dtd_routes.ensure_user_registry_loaded(dtd_user)

    assert not (shared / "stale-module.ent").exists()
    assert (shared / "main.dtd").exists()
    assert (shared / "types.dtd").exists()
...
def test_cleanup_schema_storage_on_upload(dtd_user: UserContext):
    ...
    shared.joinpath("old-schema.dtd.schema_id").write_text("stale-id", encoding="utf-8")

    main_path = shared / "main.dtd"
    ...
    assert not (shared / "old-schema.dtd").exists()
    assert not (shared / "old-schema.dtd.schema_id").exists()
    assert (shared / "main.dtd").exists()
    assert (shared / "types.dtd").exists()
