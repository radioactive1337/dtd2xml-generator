...
_STRUCTURE_DIFF_SYSTEM_PROMPT = (
    ...
    "Ты НЕ ищешь различия сам — они уже найдены. Твоя задача — дать краткую "
    "общую оценку документа целиком, а не разбирать каждый элемент по отдельности:\n"
    "1. Одним-двумя предложениями опиши суть расхождений: чем этот документ в целом "
    "отличается от эталонов (новая функциональность, расширение схемы, лишние или "
    "переименованные поля, вероятные опечатки и т.п.).\n"
    "2. Если уникальных элементов много — сгруппируй их по смыслу, а не перечисляй все.\n"
    "3. Обязательно используй точные имена XML-элементов и атрибутов из путей "
    "и фрагментов (например, <TurnoverDebitNational>, <Amount>, а не описательные "
    "фразы вроде «детализированный элемент для национальных сумм»).\n"
    "4. Заверши одной общей рекомендацией по документу: обновить эталоны, "
    "исправить/переименовать поля или требует уточнения.\n"
    "Пиши очень кратко и по-деловому, на русском. Не более 4–5 предложений всего. "
    "Без вступлений, без повтора исходного XML, без markdown-разметки и блоков кода."
)
...
class LLMService:
    ...
    def _build_structure_diff_prompt(
        ...
        *,
        ...
        lines.append("")
        lines.append(
            "Дай краткую общую оценку документа целиком (не разбирай каждый путь "
            "отдельно): в чём суть расхождений с эталонами с учётом коэффициента "
            "Жаккара (похожесть) и одна общая рекомендация. Используй точные имена "
            "элементов и атрибутов из путей, не придумывай описательные синонимы. "
